window.SocialSharingDataProvider = {
      ...
      WATSAPP: {
        name: "watsapp",
        getPopupUrl: function(encoded_title, url, img) {
            let surl = "whatsapp://send?text=" + encoded_title + "%20" + encodeURIComponent(url);
      ...
      MESSENGER: {
        name: "messenger",
        getPopupUrl: function(encoded_title, url, img) {
            let surl = "fb-messenger://share?link=" + encodeURIComponent(url);
            window.top.location = surl;
        }
      },
};
...
$('document').ready(function() {
	...
	$('body').on('click touch', 'button[custom-sharing-button]', function() {
		...
		if (provider) {
			if (shareType == 'watsapp' || shareType == 'messenger') {
				provider.getPopupUrl(title, url, img)
			} else {
